...
impl ExecutionContext {
    ...
    // Addressing modes
    fn imm(&mut self) -> AddressData<u8> {
        let address = u16::from(self.fetch_byte());
        AddressData {
            address,
            data: self.read8(address)
        }
    }
    fn imm16(&mut self) -> AddressData<u16> {
        let addr = self.fetch_word();
        AddressData {
            address: addr,
            data: self.read16(addr)
        }

    }
    fn abs(&mut self) -> AddressData<u16> {
        let addr = self.fetch_word();
        AddressData {
            address: addr,
            data: u16::from(self.read8(addr)),
        }
    }
    fn abs_x(&mut self) -> AddressData<u16> {
    ...
    fn abs_y(&mut self) -> AddressData<u16> {
    ...
    fn zp_x (&mut self) -> AddressData<u8> {
        let address = (self.fetch_word() + u16::from(self.cpu.reg.x)) & 0xff;
        AddressData {
            address,
            data: self.read8(address)
        }
    }
    fn zp_y (&mut self) -> AddressData<u8> {
        let address = (self.fetch_word() + u16::from(self.cpu.reg.y)) & 0xff;
        AddressData { address, data: self.read8(address) }
    }
    // TODO used for JMP check if fetch_word is correct
    fn indirect(&mut self) -> AddressData<u16> {
    ...
    fn indirect_x(&mut self) -> AddressData<u16> {
    ...
    fn indirect_y(&mut self) -> AddressData<u16> {
        let address = self.fetch_byte();
        AddressData {
            address: address as u16,
    ...
    pub fn decode(&mut self) {
        ...
        match opcode {
            ...
            0x10 => self.bpl(self.fetch_byte().into()),
            0x11 => self.ora(self.indirect_y().into()),
    ...
    fn bcs(&mut self, offset: u8) {
        if self.cpu.flags.carry {
            self.adv_pc(u16::from(offset as u16));
        }
        self.check_branch();
    }
    fn bcc(&mut self, offset: u8) {
        if !self.cpu.flags.carry {
            self.adv_pc(u16::from(offset));
        }
        self.check_branch();
    }
    // Branch on Equal
    ...
    fn jmp(&mut self, value: AddressData<u16>) { self.cpu.reg.pc = value.data; }
